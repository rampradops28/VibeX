...
export const followUnFollowUser = async(req,res) => {
    try{
        ...
        const currentUser = await User.findById({_id : req.user._id}) // to check the currentuser avaiable

        if(id === req.user._id){
            return res.status(400).json({error : "You can't unfollow/follow by yourself"})
        }
...
export const getSuggestedUsers = async(req,res) => {
    try{
        ...
        const fillteredUser = users.filter((user) => !userFollowedByMe.following.includes(user))
        ...
        console.log(`Error in suggested user controller:${error}`);
        res.status(400).json({error : "Internal server error"})
    }
}
...
export const updateUser = async (req,res) => {
    try{
        ...
        if((!newPassword && currentPassword) || (!currentPassword && newPassword)){
            return res.status(200).json({error : "Please provide both the new password and current password"})
        }
        ...
        if(profileImg){
            //https://res.cloudinary.com/dorrkgxal/image/upload/v1726817523/cld-sample-5.jpg
            if(user.profileImg){
        ...
        if(coverImg){
            ...
            const uploadedResponse = await cloudinary.upload.upload(coverImg)
